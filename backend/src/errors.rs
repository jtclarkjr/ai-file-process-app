...
impl IntoResponse for AppError {
  fn into_response(self) -> Response {
    let (status, error_message) = match &self {
      ...
      AppError::UnsupportedFileType(_) => (StatusCode::UNSUPPORTED_MEDIA_TYPE, self.to_string()),
      AppError::MimeTypeMismatch { .. } => (StatusCode::UNSUPPORTED_MEDIA_TYPE, self.to_string()),
      ...
      AppError::ProviderNotConfigured(_) => (StatusCode::SERVICE_UNAVAILABLE, self.to_string()),
